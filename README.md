...
- 단기 사용 목적으로 빠른 제작을 위해 단일 파일로 작성되었습니다.
- 이미지는 /image 하위 경로에 존재합니다.  
- 동아리 모집 정보글의 본문은 /contents 하위 경로에 존재합니다.  
- 상세 페이지는 data.json의 keyword를 사용하여 이미지와 본문 정보를 가져옵니다.  

## 기여

1. `./templates/Gemini Data Preprocessing.txt` 프롬프트로 json 데이터를 생성합니다.  
2. 생성한 json 데이터를 `data.json` 에 추가합니다.  
3. `./contents/{keyword}.txt`에 홍보 게시글 원문을 작성합니다.  
4. `./image/{keyword}1~9.jpg`로 홍보 게시글의 이미지를 저장합니다.  
